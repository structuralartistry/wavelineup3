def update_past_visits
  Visit.all.each do |visit|
    ...
    set_gateways.each do |set_visit_gateway|
      ...
      if set_gateway_text && !set_gateway_text.empty? && !set_gateway_text.include?('APEX') && !set_gateway_text.include?('C CX')
        normalized_gateway = 'gateway_' + set_gateway_text.downcase.gsub(/\//,'_')
        normalized_gateway = 'dominant_occiput' if normalized_gateway == 'gateway_occ'
