class ReportsController < ApplicationController
  ...
  def show
    ...
    if !filter_practice_member || !@lookback_days
      ...
      if filter_practice_member == 'all'
        ...
        @visits = Visit.joins(:practice_member => :practice).where('practice_members.practice_id=? AND visits.practice_member_id=? AND visits.date>=?', current_user.practice_id, filter_practice_member, Time.zone.now-@lookback_days.to_i.days).order('practice_members.last_name, practice_members.first_name, visits.date DESC')
      end
